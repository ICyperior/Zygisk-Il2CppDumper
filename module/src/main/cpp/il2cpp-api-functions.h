#ifndef DO_API_NO_RETURN
#define DO_API_NO_RETURN(r, n, p) DO_API(r,n,p)
#endif


DO_API(int, _ZThn20__, (const char* domain_name));

DO_API(const Il2CppImage*, _ZThn38__, ());

// assembly
DO_API(const Il2CppImage*, _ZNKSt82__rpm, (const Il2CppAssembly * assembly));

// class
DO_API(Il2CppClass*, _Znwm74dSes6, (const Il2CppImage * image, const char* namespaze, const char *name));
DO_API(FieldInfo*, _ZNSt24xoyghwnRx, (Il2CppClass * klass, void* *iter));
DO_API(Il2CppClass*, _ZNSt11__ygpypltLqtJa, (Il2CppClass * klass, void* *iter));
DO_API(const PropertyInfo*, _ZNKSt28__LOrUnxvproperties, (Il2CppClass * klass, void* *iter));
DO_API(const MethodInfo*, _ZNKSt61__9Iyfwcxjrd2m, (Il2CppClass * klass, void* *iter));
DO_API(const MethodInfo*, _ZNSt28__7GdDgt3ZoVr8wpdH, (Il2CppClass * klass, const char* name, int argsCount));
DO_API(const char*, _ZSt664ng8BcrLoHD, (Il2CppClass * klass));
DO_API(const char*, _ZNSt92ugnYnvvTGa, (Il2CppClass * klass));
DO_API(Il2CppClass*, _ZThn52nfOYnwnD, (Il2CppClass * klass));
DO_API(bool, _Znwm97__5GMQy, (const Il2CppClass * klass));
DO_API(int, _ZNSt15__J, (const Il2CppClass * klass));
DO_API(Il2CppClass*, _ZNSt80vElMdyzdY, (const Il2CppType * type));
DO_API(const Il2CppType*, _ZNSt72, (Il2CppClass * klass));
DO_API(bool, _ZNKSt74TutWKSw_ofD, (const Il2CppClass * klass));
// domain
DO_API(Il2CppDomain*, _ZSt11hcEXbjEqget, ());
DO_API(const Il2CppAssembly**, _ZNKSt41brqNPbggAzk8RuswumT, (const Il2CppDomain * domain, size_t * size));

// field
DO_API(int, _ZThn57__zhInNsR8d, (FieldInfo * field));
DO_API(const char*, _ZNSt8__TJb6, (FieldInfo * field));
DO_API(size_t, _Znwm84__FJffUr, (FieldInfo * field));
DO_API(const Il2CppType*, _ZThn15__KwhtpU, (FieldInfo * field));
DO_API(void, _Znwm10__ziO2E, (FieldInfo * field, void *value));

// method
DO_API(const Il2CppType*, _Znwm64nfNbTOJU6wspZ, (const MethodInfo * method));
DO_API(const char*, _ZThn45y4cHVOdz9f, (const MethodInfo * method));
DO_API(uint32_t, _ZThn68__ikU5X0xBpMsrh, (const MethodInfo * method));
DO_API(uint32_t, _ZSt4__h5cgzIj2cj, (const MethodInfo * method, uint32_t * iflags));
DO_API(const char*, _ZNKSt627zZLdXcm78cx2, (const MethodInfo * method, uint32_t index));

DO_API(const MethodInfo*, _Znwm84nzkIg_E, (PropertyInfo * prop));
DO_API(const MethodInfo*, _ZNSt23__xgrJxIqr_btq, (PropertyInfo * prop));
DO_API(const char*, _ZNSt19__EpRk0E, (PropertyInfo * prop));

DO_API(Il2CppString*, _ZNKSt93__vqxXpO, (const char* str));
DO_API(Il2CppThread*, _Znwm27vHddCzmNzdmn, (Il2CppDomain * domain));
DO_API(bool, _ZNSt45zFeGW, (Il2CppThread * thread));

DO_API(const char*, _ZNKSt38hUVpzbrZiR, (const Il2CppImage * image));
DO_API(size_t, _ZSt11__NhBy1jhna, (const Il2CppImage * image));

DO_API(const Il2CppClass*, _Znwm86__E5KFW, (const Il2CppImage * image, size_t index));
DO_API(const Il2CppType*, _ZThn73TTr, (const MethodInfo * method, uint32_t index));
DO_API(Il2CppClass*, _ZThn43Obbdw30OAUbZQE6J1Bx8, (Il2CppReflectionType * type));
